...
def get_dyna_model_spec(params):
  model = Dyna_MODEL(params)
  print('# of parameters: ', sum([p.numel() for p in model.parameters()]))
  optimizer = optim.Adam(model.parameters(), params.learning_rate)
  lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer,T_max=params.train_epochs,eta_min=4e-08)
  loss_fn = torch.nn.L1Loss()
...
class Dyna_MODEL(nn.Module):
    def __init__(self,params):
        super(Dyna_MODEL, self).__init__()
        self.network1 = MODEL(params,4,2)
        self.network2 = MODEL(params,2,2)

    def forward(self, x,params):
        if x.shape[2] < params.global_size:
            out = self.network1(x)
        else:
            out = self.network2(x)
        return out
...
class MODEL(nn.Module):
  ...
  def __init__(self, params,blocks,width):
    ...
    for _ in range(self.num_blocks):
      body.append(
          Block(
              ...
              res_scale=1 / math.sqrt(self.num_blocks),
          ))
    conv = weight_norm(
        nn.Conv2d(
            params.num_residual_units,
