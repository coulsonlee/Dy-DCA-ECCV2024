...
import math
import thop
from datasets.vsd4k import DIV2K_
train_chunk = ''
if __name__ == '__main__':
    ...
    model, criterion, optimizer, lr_scheduler, metrics = model_module.get_dyna_model_spec(
        params)
    ...
    def train(epoch):
        ...
        s_img = []
        l_img = []
        for batch_idx, (s1,s2) in enumerate(train_data_loader):
            if len(s_img) != 16:
                s_img.extend(s1)
            else:
                for (data,target) in s_img:
