...
class DIV2K_(torch.utils.data.Dataset):
    ...
    #         if lr_image.shape[1]<70:
    #             grouped_batch['s'].append((lr_image,hr_image))
    #         else:
    #             grouped_batch['l'].append((lr_image,hr_image))
    #     print(len(grouped_batch['s']))
    #     print(len(grouped_batch['l']))
    #     return grouped_batch
    def collate_fn(batch,params):
        size1_images = []
        size2_images = []
        for lr_image, hr_image in batch:
            if lr_image.shape[1]<params.global_size:
                size1_images.append((lr_image,hr_image))
            else:
                size2_images.append((lr_image,hr_image))
        return size1_images, size2_images
